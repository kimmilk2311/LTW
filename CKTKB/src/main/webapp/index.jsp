<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Index</title>
<!--Bootstrap-->
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css">

<link rel="stylesheet" href="css/index.css"/>
</head>
<body>
      <div class="trangchu">
            <div class="overlap-group-wrapper">
                <div class="overlap-group">
                    <img class="rectangle" src="img/rectangle-1.png" />
                    <img class="line" src="img/line.png" />
                   
                    <button type="submit"  onclick="location.href='lop'" class="dangkytkb btn" >
                       <img class="image" src="img/image-3.png" />
                   </button>
                   <button type="button" onclick="location.href='mon'" class="thongtindangnhap btn">
                      <img class="img" src="img/image-4.png" />
                   </button>
                    <header class="header">
                        <p class="UNVIERSITY-OF">UNVIERSITY OF SCIENECE AND EDUCATION<br />The University of Da Nang</p>
                        <img class="image-3" src="img/image-1.png" />
                    </header>
                    <div class="register-schedule">Thông tin<br />Lớp Học</div>
                    <div class="teacher-information">Thông tin <br />Môn Học</div>
                </div>
            </div>
        </div>
      
</body>
</html>